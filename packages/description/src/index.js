...
import { isFunction } from 'setaria-ui/src/utils/types';
  ...
  methods: {
    ...
    getRows() {
      const children = ((this.$slots.default || []).filter(vnode => vnode.tag &&
            vnode.componentOptions && vnode.componentOptions.Ctor.options.name === 'ElDescriptionItem'));
      ...
      nodes.forEach((node, index) => {
        ...
        if (span >= this.column) {
          rows.push(temp);
          rows.push([this.filledNode(node, span, span, true)]);
          count = this.column;
          temp = [];
        } else if (span < count) {
          count -= span;
